...
from keyboards.inlines.accses import true_false, follow_button, like_button, view_button, comment_button, payment_type_select

API_TOKEN = '6762535664:AAE3Hj6U2Tbse-sG5AJkL02slEA_xlPiLkQ'
...
async def tasdiq_followers(call: types.CallbackQuery):
    ...
    await bot.send_message(5035280819,
    ...
    await call.message.answer("To`lov turini tanlang!:", reply_markup=payment_type_select)
...
@dp.callback_query_handler(text='uzcard_pay_card')
async def select_payment_type(call: types.CallbackQuery, state: FSMContext):
    await call.message.answer(f"""Kartaga {son[call.message.chat.id] * 6}so'm va checkni yuboring:
        Karta raqami: <code>1111 1111 1111 1111</code>""")
    await Shogirdchalar.payment_check_check.set()


@dp.callback_query_handler(text='xumo_pay_card')
async def select_payment_type(call: types.CallbackQuery, state: FSMContext):
    await call.message.answer(f"""Kartaga {son[call.message.chat.id] * 6}so'm va checkni yuboring:
        Karta raqami: <code>2222 2222 2222 2222</code>""")
    await Shogirdchalar.payment_check_check.set()


@dp.message_handler(state=Shogirdchalar.payment_check_check, content_types=types.ContentTypes.PHOTO)
async def check_send_to_admin(message: Message, state: FSMContext):
    file = message.photo
    instagram_nomi = user_instagram[f"{message.chat.id}"]
    print(file[0])
    await bot.send_photo(5035280819, file[0]['file_id'], caption=f"Buyurtmachi: {message.from_user.username}\nTanlov Turi: Comment\nSoni: {son[message.chat.id]}\nInstagram:{instagram_nomi}\nNarx:{son[message.chat.id] * 6}")
    await state.finish()
